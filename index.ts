...
function hookText(){
    ...
    const assembly = Il2Cpp.domain.assembly("ScriptsFramework.Common");
    const TextEncryptUtil = assembly.image.class("Common.TextEncryptUtil.TextEncryptUtil");
    // Hook the specific function
Interceptor.attach(Module.findExportByName("GameAssembly.dll", 
    "Common_TextEncryptUtil_TextEncryptUtil__FGNACANOIEC"), {
    onEnter: function(args) {
        console.log("\n=== Common_TextEncryptUtil_TextEncryptUtil__FGNACANOIEC Called ===");
        
        // arg[0] is the MethodInfo*, arg[1] is the System_String_o* (HACPDJNBPKI)
        this.encryptedString = args[0];
        
        if (this.encryptedString) {
            try {
                const encryptedText = Il2Cpp.string(this.encryptedString);
                console.log("Encrypted Input: " + encryptedText);
                console.log("Encrypted Input Length: " + encryptedText.length);
            } catch (e) {
                console.log("Error reading encrypted string: " + e);
            }
        }
    ...
    onLeave: function(retval) {
        ...
        if (retval) {
            try {
                const decryptedText = Il2Cpp.string(retval);
                console.log("Decrypted Output: " + decryptedText);
                console.log("Decrypted Output Length: " + decryptedText.length);
                
                // You can also modify the return value if needed
                // const newString = Il2Cpp.string("modified text");
                // retval.replace(newString);
            } catch (e) {
                console.log("Error reading decrypted string: " + e);
            }
        }
...
// Il2Cpp.perform(getKeyIV).catch(console.error);
